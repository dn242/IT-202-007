	...
	<div class="row" style="margin-top:100px;">
		<div class="col-12 text-center">
			<?php
				$db = getDB();
				$index=0;
				$stmt = $db->prepare("SELECT Y_Orderitems.created AS date, Y_Orderitems.quantity AS orderQuantity, Y_Products.name as productName, Y_Products.category as category, Y_Products.price as price, Y_Orders.user_id as id  FROM (Y_Orderitems JOIN Y_Orders ON Y_Orderitems.order_id = Y_Orders.id) JOIN Y_Products ON Y_Orderitems.product_id =Y_Products.id ");
				$stmt->execute([
				]);
								...
								if($result!=false){
									foreach($result as $value){
										if(get_user_id() == $value["id"]){
											echo "<tr><td>".($index+1).".</td><td>".$value["productName"]."</td><td>".$value["category"]."</td><td>".$value["orderQuantity"]."</td><td>".$value["price"]."$</td><td>Ordered on ".$value["date"]."</td></tr>";
											$index++;
										}
									}
								}
